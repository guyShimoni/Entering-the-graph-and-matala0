...
/**
 * This interface represents a general Polynom: f(x) = a_1*X^b_1 + a_2*X^b_2 ... a_n*Xb_n,
 ...
 * 
 * 2. Math:
 * 2.1 void add(Polygon p) // add p to this Polynon//חיבור
 * 2.2 void subtract(Polygon p) // subtract p from this Polygon//לחסר
 * 2.3 void multiply(Polygon p) // multiply this Polygon by p//להכפל
 * 
 * 3. Utils
 * 3.1 isZero()
 * 3.2 Polynom derivative() // returns a new Polygon of the derivative ("NIGZERET").//נגזרת פולינום
 * 3.3 double f(x) // return this Polygon value at p(x) //חישוב צלע מהצורה/פוליגון
 * 3.4 boolean equals(Polygon p) // returns true iff for any x: this.f(x) == p.f(x)//השוואת צלעות
 * 3.5 double root(double x0, double x1, double eps) // assuming (f(x0)*f(x1)<=0, returns f(x2) such that:
 *													//	(i) x0<=x2<=x1 & (ii) {f(x2)<eps //חישוב שורש 
...
public interface Polynom_able extends cont_function{
	 ...
	 * @param p1
	 */
	public void substract(Polynom_able p1);//חיסור
	/**
	 * Multiply this Polynom by ap1
	 * @param p1
	 */
	 ...
	 * @return 
	 */
	public Polynom_able copy();//העתקה
	/**
	 ...
	 */
	/*public double area(double x0,double x1, double eps);//שטח
	/**
